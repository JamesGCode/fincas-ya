...
export function ImagePreviewModal({
  ...
  const handleClose = () => {
    ...
    setTimeout(() => {
      onClose();
    }, 250); // match duration-200 or 300
  };

  // Initialize from initialFiles exactly ONCE physically or if initialFiles changes entirely.
  // The user reported adding files breaks old ones.
  // The issue was:
  // 1. `useEffect(() => { ... URL.revokeObjectURL ... }, [items]);` was revoking ALL existing object URLs whenever `items` changed.
  // We need to only revoke when the component unmounts or explicitly when an item is removed.
  ...
  useEffect(() => {
    ...
    // Cleanup on unmount for initial files won't strictly work this way if we modify the array,
    // so we handle cleanup via a master unmount ref.
  }, [initialFiles]);
  ...
  const handleAddFiles = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      setCurrentIndex(items.length); // auto select the first newly added
    }
  ...
  const handleRemove = (e: React.MouseEvent, idToRemove: string) => {
    ...
    // Revoke the URL for the removed item to free memory
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-hidden relative flex items-center justify-center p-4 min-h-0 shrink">
          {currentItem && (
            <img
              key={currentItem.id} // forces re-render/animation on mount
        ...
        <div className="bg-background border-t w-full shrink-0 flex flex-col">
          <div className="max-w-3xl w-full mx-auto px-4 pb-4 pt-4 flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              {/* The send button follows the right input visually like WhatsApp */}
              <button
