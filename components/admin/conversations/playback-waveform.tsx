...
export function PlaybackWaveform({
  ...
  useEffect(() => {
    ...
    const generateWaveform = async () => {
      try {
        ...
        waveDataRef.current = Array.from(
          { length: 40 },
  ...
  const drawWaveform = () => {
    ...
    // Compute CSS colors from custom properties
    const styles = getComputedStyle(canvas);
    const playedColor =
      styles.getPropertyValue("--color-bubble-wave-played").trim() ||
      "rgba(255,255,255,0.9)";
    const unplayedColor =
      styles.getPropertyValue("--color-bubble-wave-unplayed").trim() ||
      "rgba(255,255,255,0.35)";
    const dotColor =
      styles.getPropertyValue("--color-bubble-wave-dot").trim() || "#00d0ff";
    ...
    // Draw the progress dot exactly aligned with progress percentage
    ...
    // Add a slight drop shadow / inner glow to the dot to make it pop like the screenshot
    ctx.fillStyle = "rgba(255, 255, 255, 0.2)";
    ctx.beginPath();
    ctx.arc(currentX, height / 2, 2.5, 0, 2 * Math.PI);
    ctx.fill();
  };
  ...
  return (
    <canvas
      ...
      aria-label="Forma de onda de la grabacion"
    />
  );
}
