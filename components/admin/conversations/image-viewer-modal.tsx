...
export function ImageViewerModal({
  ...
  // Manejo de teclado para cerrar y navegar
  useEffect(() => {
  ...
  }, [onClose, currentIndex, images.length]); // Depend on currentIndex so handlers get the latest state indirectly via handleNext reference closure if needed (though handleNext below uses setState callback)
  ...
  return createPortal(
    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/90 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col h-full w-full">
        {/* Contenedor principal de la imagen expandida */}
        <div
        ...
        >
          ...
          {images.length > 1 && (
            <>
              <Button
                ...
                className="absolute left-4 top-1/2 -translate-y-1/2 z-110 text-white/70 hover:text-white hover:bg-white/10 rounded-full h-12 w-12 hidden sm:flex"
              >
              ...
              <Button
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 z-110 text-white/70 hover:text-white hover:bg-white/10 rounded-full h-12 w-12 hidden sm:flex"
              >
          ...
          <img
            key={images[currentIndex].url} // Force re-render for animation on change
        ...
        {images.length > 1 && (
          <div className="w-full shrink-0 flex flex-col z-110">
            <div className="max-w-3xl w-full mx-auto px-4 pb-4 pt-4 flex flex-col gap-4">
              <div className="flex items-center justify-center overflow-x-auto pb-2 scrollbar-hide gap-2">
                {images.map((img, idx) => (
                  <div
                    key={img.url + idx}
